...
pub fn select_commit_range(commits: &[CommitInfo]) -> Result<(usize, usize)> {
    ...
    println!("{}", "-".repeat(80).cyan());
    println!("\n{}", "Enter range in format 'start-end' (e.g., '5-11'):".bold().green());
    print!("{} ", "Range:".bold());
    io::stdout().flush()?;
...
pub fn show_range_details(commits: &[CommitInfo], start_idx: usize, end_idx: usize) -> Result<()> {
    ...
    for (idx, commit) in commits[start_idx..=end_idx].iter().enumerate() {
        println!(
            ...
            commit.oid.to_string()[..8].to_string()
        );
        println!(
            "{}: {}",
            "Author".bold(),
...
pub fn rewrite_range_commits(args: &Args) -> Result<()> {
    ...
    for (i, (commit, timestamp)) in commits[start_idx..=end_idx].iter().zip(timestamps.iter()).enumerate() {
        println!(
            "  {}: {} -> {}",
    ...
    // Apply changes
    apply_range_changes(args, &commits, start_idx, end_idx, &author_name, &author_email, &timestamps)?;
...
mod tests {
    ...
    fn test_generate_range_timestamps() {
        ...
        let end = NaiveDateTime::parse_from_str("2023-01-01 10:00:00", "%Y-%m-%d %H:%M:%S").unwrap();
    ...
    fn test_rewrite_range_commits_with_repo() {
        ...
        let end_time = NaiveDateTime::parse_from_str("2023-01-01 10:00:00", "%Y-%m-%d %H:%M:%S").unwrap();
